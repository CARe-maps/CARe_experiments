...
class PointProjector:
    def __init__(self, camera: Camera, 
        ...
        self.masks = masks
        self.topk_calculation_down_ratio = res_down_ratio   
    ...
    def get_visible_points_view(self):
        ...
        for i, idx in tqdm(enumerate(indices)): # for each view
            ...
            inside_mask = (projected_points[i,:,0] >= 0) * (projected_points[i,:,1] >= 0) \
                                * (projected_points[i,:,0] < width) * (projected_points[i,:,1] < height)

            # print(f"Number of points inside the mask: {inside_mask.sum()}")
    ...
    def get_bbox(self, mask, view):
        if(self.visible_points_in_view_in_mask[view][mask].sum()!=0):
            ...
            # true_values = ((np.array(true_values) + 0.5) * self.topk_calculation_down_ratio).astype(int)
            # crop by resolution
            # true_values[0][true_values[0] >= self.resolution[0]] = self.resolution[0] - 1
            # true_values[1][true_values[1] >= self.resolution[1]] = self.resolution[1] - 1
            valid = True
            t, b, l, r = true_values[0].min(), true_values[0].max()+1, true_values[1].min(), true_values[1].max()+1 
            # t, b, l, r = t*self.topk_calculation_down_ratio, b*self.topk_calculation_down_ratio, l*self.topk_calculation_down_ratio, r*self.topk_calculation_down_ratio
        else:
            valid = False
            t, b, l, r = (0,0,0,0)
    ...
    def get_visible_points_in_view_in_mask(self):
        ...
        visible_points_in_view_in_mask = np.zeros((num_view, masks.num_masks, \
            math.ceil(resolution[0]/self.topk_calculation_down_ratio), math.ceil(resolution[1]/self.topk_calculation_down_ratio)), dtype=bool)
        ...
        for i in tqdm(range(num_view)):
            for j in range(masks.num_masks):
                ...
                proj_points = (projected_points[i][visible_masks_points] / self.topk_calculation_down_ratio).astype(int)
        ...
        print(self.visible_points_in_view_in_mask.shape)
...
class FeaturesExtractor:
    ...
    def extract_features(self, topk, multi_level_expansion_ratio, num_levels, num_random_rounds, num_selected_points, save_crops, out_folder, optimize_gpu_usage=False):
        ...
        mask_clip = np.zeros((num_masks, topk, 768)) #initialize mask clip
        ...
        for mask in tqdm(range(num_masks)): # for each mask 
            ...
            for view_count, view in enumerate(topk_indices_per_mask[mask]): # for each view
                ...
                visible_points_in_view_in_mask_raw = self.point_projector.visible_points_in_view_in_mask[view][mask].astype(np.uint8) * 255
                # read as image and resize to resolution and convert to bool
                resolution = self.point_projector.resolution
                visible_points_in_view_in_mask = Image.fromarray(visible_points_in_view_in_mask_raw).resize((resolution[1], resolution[0]))
                # if save_crops:
                #     visible_points_in_view_in_mask.save(os.path.join(out_folder, f"mask{mask}_{view}.png"))
                visible_points_in_view_in_mask = np.array(visible_points_in_view_in_mask) > 128

                # Get original mask points coordinates in 2d images
                point_coords = np.transpose(np.where(visible_points_in_view_in_mask == True))
                if (point_coords.shape[0] > 0):
                    ...
                    for level in range(num_levels):
                        ...
                        if(save_crops):
                            # self.images.images[view].save(os.path.join(out_folder, f"original{mask}_{view}.png"))
            ...
            if(len(images_crops) > 0):
                ...
                with torch.no_grad():
                    ...
                    mask_clip[mask, :image_features.shape[0]] = image_features.cpu().numpy()
